...
function mostrarCamposTA(indice, tipoId) {
  ...
  if (tipoId) {
    ...
    if (tipoId === tipoTramiteId) {
      ...
      if (taCorreoInput && !taCorreoInput.value) taCorreoInput.value = correo;
    }
  } else {
...
if (!empty($av['folio_salida_numero'])) {
    $folio_salida = str_pad($av['folio_salida_numero'], 3, '0', STR_PAD_LEFT) . '/' . $av['folio_salida_anio'];
        ...
        $subtramite_id = isset($av['subtramite_id'])    ? $av['subtramite_id']       : '';
      ?>
      <tr>
...
<section id="tramite" class="tramite-box mb-4">
  ...
  <div id="paso1-seleccion">
    ...
    <div class="row g-3 mb-4">
      ...
      $numOfIdStmt=$conn->prepare("SELECT id FROM tipos_tramite WHERE codigo='NUM_OFICIAL' LIMIT 1");
      $numOfIdStmt->execute();
      $numOfIdRes=$numOfIdStmt->get_result()->fetch_assoc();
      ...
      $avgStmt->close();
      $promDuracion='10 días hábiles';
      $promSalida='';
       if($avgData && (int)$avgData['n']>0){
         $seg=(int)round($avgData['avg_seg']??0);
         if($seg<0)$seg=0;
         $d=floor($seg/86400);
         $h=floor(($seg%86400)/3600);
         $m=floor(($seg%3600)/60);
        $dur=trim(($d>0?"{$d}d ":'').($h>0?"{$h}h ":'').($m>0?"{$m}m":''))?:'0m';
        $promDuracion="Resolución prom: {$dur} <small class=\"d-block\" style=\"font-size:.62rem;\">(n={$avgData['n']})</small>";
        $ssecs=(int)round($avgData['avg_salida_secs']??0);
        $sh=floor($ssecs/3600);
        $sm=floor(($ssecs%3600)/60);
        $promSalida=sprintf('%02d:%02d',$sh,$sm);
      }
      $iconos = [
          ...
          <div class="card-body p-3">
            <div class="d-flex align-items-center gap-3 mb-2">
              ...
              <div>
                 ...
                 <?php $tiempoLabel = ($tp['id'] == 1) ? $promDuracion : '10 días hábiles'; ?>
                 <div class="text-muted" style="font-size:.72rem; line-height:1.1;"><?= $tiempoLabel ?></div>
                 <?php if ($tp['id'] == 1 && !empty($promSalida)): ?>
                 <div class="text-muted" style="font-size:.62rem;">Salida prom ~ <?= htmlspecialchars($promSalida) ?></div>
                 <?php endif; ?>
              </div>
            </div>
  ...
  <div class="table-responsive">
    <table id="tablaCorreccion" class="table table-bordered table-hover align-middle">
      ...
      <?php while($t=$correccion_res->fetch_assoc()):
        ...
        $subtramite_id = isset($t['subtramite_id'])        ? $t['subtramite_id']      : '';
      ?>
        <tr>
